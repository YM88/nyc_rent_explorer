...
server <- function(input, output, session) {
    ...
    ts_input <- reactive({
      df %>% filter(Borough == input$boro & Neighborhood == input$neighborhood)
    ...
    output$ts_plot <- renderPlot({
      ...
      ggplot(ts_input(), aes_string(x = "Date", y = input$type)) +
        geom_line() +
...
```{r, fig.width=10}
df %>%
  filter(Neighborhood == "Astoria" & Borough == "Queens") %>%
  ggplot(aes_string(x = "Date", y = "`One Bedroom`")) +
  geom_line() +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  scale_x_date(breaks = scales::pretty_breaks(n = 10), date_labels = "%b-%Y")
```
