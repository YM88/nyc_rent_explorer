...
server <- function(input, output, session) {
    ...
    # Here is the histogram plot. It allows for user to compare two boroughs, compare a borough with a neighborhood, compare two neighborhoods. It DOES NOT yet prevents output where there are less than ten data points for either histogram.
    output$histogram <- renderPlotly({
      if(input$neighborhood_compare == "None" & input$neighborhood_compare2 == "None"){
        ...
        add_histogram(x = ~(hist_filterBoro() %>% filter(Borough == input$boro_compare2) %>% pull(input$type_compare2)), name = paste(input$type_compare2, "in", input$boro_compare2)) %>% 
