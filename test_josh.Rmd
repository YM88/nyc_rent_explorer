...
plot_ly(alpha = 0.6) %>% 
  add_histogram(x = ~(df %>% filter(Neighborhood == "") %>% pull(input$type_compare)), name = paste(input$type_compare, "in", input$neighborhood_compare))



df %>% 
  filter(Borough %in% c("Manhattan", "Bronx")) %>% group_by(Borough, Date) %>% summarize(Studio = mean(Studio, na.rm = T),One_Bedroom = mean(One_Bedroom, na.rm = T),Two_Bedrooms = mean(Two_Bedrooms, na.rm = T),Three_Bedrooms = mean(Three_Bedrooms, na.rm = T)) %>% distinct(Studio) %>% filter(Borough == "Manhattan")
```
